...
class PageController extends Controller
{
    ...
    public function blogGrid(Request $request)
    {
        ...
        // Price filter - using teacherSubjects pivot table
        if (!empty($validated['min_price']) || !empty($validated['max_price'])) {
            $baseQuery->whereHas('subjects.teacherSubjects', function ($q) use ($validated) {
        ...
        if (!empty($validated['subject_name'])) {
            $baseQuery->whereHas('subjects', function ($q) use ($validated) {
                $q->where('subject_name', 'like', '%' . $validated['subject_name'] . '%');
            });
        }
        ...
        if (!empty($validated['needTeachers'])) {
            if ($validated['needTeachers'] === 'all') {
                ...
                $baseQuery->whereHas('needTeachers', fn($q) => $q->where('subject_name', 'like', '%' . $validated['needTeachers'] . '%'));
            }
        }
        ...
        $subjects = \App\Models\SubjectsOfLearningCenter::distinct()->pluck('subject_name')->filter()->sort()->values();
