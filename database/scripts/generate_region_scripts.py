#!/usr/bin/env python3
"""
Har bir viloyat uchun alohida scriptlarni generatsiya qilish
"""

import os
from regions_config import REGIONS_CONFIG

def generate_script_for_region(region_key, region_data):
    """Viloyat uchun script generatsiya qilish"""
    
    script_content = f'''#!/usr/bin/env python3
"""
{region_data["name"]} uchun o'quv markazlarini yig'ish scripti
"""
...
'''
    ...
    # Faylni yozish
    filename = f"{region_key}_learning_centers.py"
    filepath = os.path.join(os.path.dirname(__file__), filename)
    
    with open(filepath, "w", encoding="utf-8") as f:
        f.write(script_content)
    
    # Ishga tushirish huquqini berish
    os.chmod(filepath, 0o755)
    
    return filepath

def main():
    print("🚀 Viloyatlar uchun scriptlarni generatsiya qilish...")
    
    generated_scripts = []
    
    for region_key, region_data in REGIONS_CONFIG.items():
        print(f"📝 {region_data['name']} uchun script yaratilmoqda...")
        filepath = generate_script_for_region(region_key, region_data)
        generated_scripts.append(filepath)
        print(f"   ✅ {filepath}")
    
    print(f"\n🎉 {len(generated_scripts)} ta script generatsiya qilindi!")
    
    print("\n📋 Generatsiya qilingan scriptlar:")
    for script in generated_scripts:
        print(f"   📄 {os.path.basename(script)}")
    
    print(f"\n🔥 Endi barchasini parallel ishga tushirish uchun:")
