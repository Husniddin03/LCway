<?php

return [
    // Page title
    'title' => 'Все учебные центры',
    
    // Hero Section
    'hero' => [
        'title' => 'Учебные центры',
        'description' => 'Найдите лучшие учебные центры по всему Узбекистану и выберите подходящие вам курсы',
    ],
    
    // Search and Filter Section
    'search_filter' => [
        'search_placeholder' => 'Поиск учебных центров...',
        'all' => 'Все',
        'map_filter' => 'По карте',
        'map_search' => 'Поиск по карте',
        'map_subtitle' => 'Выберите ваше местоположение и введите радиус',
        'map_loading' => 'Карта загружается...',
        'locating' => 'Определение местоположения...',
        'my_location' => 'Мое местоположение',
        'map_hint' => '🖱 Нажмите на карту или перетащите маркер',
        'selected_location' => 'Выбранное местоположение',
        'search_radius' => 'Радиус поиска',
        'clear' => 'Очистить',
        'search' => 'Поиск',
        'results_found' => 'центров найдено',
        'results_radius' => 'км радиусе',
        'sort' => 'Сортировка',
        'sort_name' => 'Название',
        'sort_distance' => 'Расстояние',
        'sort_rating' => 'Рейтинг',
        'subjects' => 'Предметы',
        'type' => 'Тип',
        'teacher_announcements' => 'Объявления учителей',
        'all_announcements' => 'Все объявления',
        'results_count' => 'учебных центров найдено',
        'searching' => 'Поиск...',
        'price' => 'Цена',
        'min_price' => 'Мин. цена',
        'max_price' => 'Макс. цена',
        'apply' => 'Применить',
        'example_price' => 'Например: :price',
    ],
    
    // Learning Centers Grid
    'centers_grid' => [
        'read_more' => 'Подробнее',
        'announcement' => 'Объявление',
        'teacher_needed' => 'Нужен учитель',
        'more_announcements' => 'Еще :count объявлений. Подробности',
        'no_announcements' => 'Пока нет объявлений!',
        'no_centers_found' => 'Учебные центры не найдены',
        'clear_filters' => 'Очистить фильтры',
    ],
];
