<x-layout>
        ...
        <div class="relative max-w-7xl mx-auto">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
                ...
                <div class="lg:col-span-2 animate-slide-in-right">
                    <form action="{{ route('course.weekdayUpdate', $LearningCenter->slug) }}" method="POST" 
                        ...
                        <div class="space-y-6">
                            ...
                            @foreach ($weekdays->filter(fn($day) => $day->schedule_id) as $day)
                                <div class="group relative" data-schedule-id="{{ $day->schedule_id ?? '' }}">
                                    ...
                                    <div class="rounded-2xl p-6 border border-gray-200/50 dark:border-gray-600/50 hover:border-blue-300/50 dark:hover:border-blue-600/50 transition-all duration-300 hover:shadow-lg group/item">
                                        ...
                                        <!-- Hidden input for weekdays and schedule id -->
                                        <input type="hidden" name="days[{{ $day->id }}][weekdays]" value="{{ $day->weekdays }}" />
                                        @if($day->schedule_id)
                                            <input type="hidden" name="days[{{ $day->id }}][schedule_id]" value="{{ $day->schedule_id }}" />
                                        @endif
                                    </div>
                                </div>
                            @endforeach
                            
                            @if($weekdays->filter(fn($day) => $day->schedule_id)->isEmpty())
                                <div class="text-center py-12 bg-gray-50 dark:bg-gray-800/50 rounded-2xl border border-dashed border-gray-300 dark:border-gray-700">
                                    <div class="w-16 h-16 bg-gray-100 dark:bg-gray-700 rounded-full flex items-center justify-center mx-auto mb-4">
                                        <svg class="w-8 h-8 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                        </svg>
                                    </div>
                                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">Jadval hali qo'shilmagan</h3>
                                    <p class="text-gray-600 dark:text-gray-400 mb-4">Hafta kunlarini qo'shish uchun yuqoridagi tugmani bosing</p>
                                </div>
                            @endif
                        </div>
    ...
    <div id="addWeekdayModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center p-4">
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 max-w-md w-full shadow-2xl">
            ...
            <form id="addWeekdayForm" onsubmit="addWeekday(event)">
                ...
                <div class="space-y-4">
                    <div>
                        <label for="new_weekdays" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            {{ __('course-weekday.form.weekdays_label') }}
                        </label>
                        <select id="new_weekdays" name="weekdays" required
    ...
    <script>
        ...
        function addWeekday(event) {
            ...
            const learningCenterSlug = '{{ $LearningCenter->slug }}';
            
            fetch(`/center/weekdayAdd/${learningCenterSlug}`, {
                method: 'POST',
                body: formData,
                headers: {
