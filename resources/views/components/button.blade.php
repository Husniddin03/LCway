...
@php
    ...
    $variantClasses = match ($variant) {
        'primary'
            => 'bg-gradient-to-r from-primary-600 to-accent-600 text-white hover:scale-105 shadow-lg hover:shadow-xl focus:ring-primary-500',
        'secondary'
        ...
        'ghost'
            => 'bg-transparent text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 focus:ring-primary-500',
...
@if ($href)
    <a href="{{ $href }}" class="{{ $classes }} text-gray-900 dark:text-white"
        {{ $disabled ? 'tabindex="-1" aria-disabled="true"' : '' }}>
        {{ $slot }}
    </a>
@else
    <button type="{{ $type }}" class="{{ $classes }} text-gray-900 dark:text-white"
        {{ $disabled ? 'disabled' : '' }}>
        {{ $slot }}
    </button>
@endif
