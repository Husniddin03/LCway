...
<x-card hover class="overflow-hidden group cursor-pointer" onclick="window.location.href='{{ route('course.show', $course->id) }}'">
    <!-- Course Image -->
    <div class="relative {{ $compact ? 'h-40' : 'h-48' }} overflow-hidden">
        @if($course->image)
            <img src="{{ asset($course->image) }}" alt="{{ $course->title }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300">
        @else
        ...
        @if($showRating && $course->rating)
            <div class="absolute top-3 right-3 bg-black/70 backdrop-blur-sm text-white px-2 py-1 rounded-lg text-xs font-medium flex items-center space-x-1">
