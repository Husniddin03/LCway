...
@php
    $inputId = $id ?? $name;
    $baseClasses =
        'w-full px-4 py-3 rounded-lg border bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 transition-all duration-200 focus:outline-none focus:ring-2 focus:border-transparent';
...
<div class="space-y-2">
    ...
    <input type="{{ $type }}" name="{{ $name }}" id="{{ $inputId }}" value="{{ $value }}"
