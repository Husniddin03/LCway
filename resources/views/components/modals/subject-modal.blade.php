...
<div x-show="modals.subject" class="fixed inset-0 z-50 flex items-center justify-center" style="display: none;">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md mx-4 relative z-10">
        <div class="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
            <h3 class="font-semibold text-gray-900 dark:text-white" x-text="editing.subject ? '{{ __('user.modals.edit_subject') }}' : '{{ __('user.modals.new_subject') }}'"></h3>
        ...
        <form @submit.prevent="saveSubject()" class="p-4 space-y-4">
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('user.modals.subject_name') }}</label>
            ...
            <div class="flex gap-3 pt-2">
                <button type="button" @click="closeSubjectModal()" class="flex-1 px-4 py-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700">{{ __('user.modals.cancel') }}</button>
