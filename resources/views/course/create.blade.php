<x-layout>
                ...
                <form action="{{ route('course.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="type" id="type" class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-800 dark:text-white" required>
                                ...
                                <option value="Unversitet" {{ old('type') == 'Unversitet' ? 'selected' : '' }}>Unversitet</option>
                            </select>
                            @error('type')
                                <p class="mt-2 text-sm text-danger-600 dark:text-danger-400">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="province" id="region" class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-800 dark:text-white" required>
                                    <option value="" disabled {{ !old('province') ? 'selected' : '' }}>Viloyatni tanlang...</option>
                                    <option value="Toshkent" {{ old('province') == 'Toshkent' ? 'selected' : '' }}>Toshkent</option>
                                    <option value="Sirdaryo" {{ old('province') == 'Sirdaryo' ? 'selected' : '' }}>Sirdaryo</option>
                                    <option value="Jizzax" {{ old('province') == 'Jizzax' ? 'selected' : '' }}>Jizzax</option>
                                    <option value="Samarqand" {{ old('province') == 'Samarqand' ? 'selected' : '' }}>Samarqand</option>
                                    <option value="Buxoro" {{ old('province') == 'Buxoro' ? 'selected' : '' }}>Buxoro</option>
                                    <option value="Navoiy" {{ old('province') == 'Navoiy' ? 'selected' : '' }}>Navoiy</option>
                                    <option value="Qashqadaryo" {{ old('province') == 'Qashqadaryo' ? 'selected' : '' }}>Qashqadaryo</option>
                                    <option value="Surxandaryo" {{ old('province') == 'Surxandaryo' ? 'selected' : '' }}>Surxandaryo</option>
                                    <option value="Xorazm" {{ old('province') == 'Xorazm' ? 'selected' : '' }}>Xorazm</option>
                                    <option value="Andijon" {{ old('province') == 'Andijon' ? 'selected' : '' }}>Andijon</option>
                                    <option value="Namangan" {{ old('province') == 'Namangan' ? 'selected' : '' }}>Namangan</option>
                                    <option value="Farg'ona" {{ old('province') == 'Farg\'ona' ? 'selected' : '' }}>Farg'ona</option>
                                    <option value="Qoraqalpog'iston" {{ old('province') == 'Qoraqalpog\'iston' ? 'selected' : '' }}>Qoraqalpog'iston</option>
                                </select>
                                @error('province')
                                    <p class="mt-2 text-sm text-danger-600 dark:text-danger-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <select name="region" id="district" class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-800 dark:text-white" required>
                                    <option value="" disabled {{ !old('region') ? 'selected' : '' }}>Tumanni tanlang...</option>
                    ...
                    <div class="space-y-6">
                        ...
                        <div id="subjects-wrapper" class="space-y-4">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select name="subjects[0][id]" class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-800 dark:text-white" required>
                        ...
                        <button type="button" id="add-subject" class="text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 font-medium text-sm flex items-center">
                            ...
                            @for ($day = 0; $day < 6; $day++)
                                <div class="space-y-2">
                                    ...
                                    <div class="grid grid-cols-2 gap-2">
                                        <div>
                                            <label class="block text-xs text-gray-600 dark:text-gray-400 mb-1">Ochilishi</label>
                                            <input type="time" name="days[{{ $day }}][open_time]" value="{{ old("days.$day.open_time", '06:00') }}" 
                                        ...
                                        <div>
                                            <label class="block text-xs text-gray-600 dark:text-gray-400 mb-1">Yopilishi</label>
                                            <input type="time" name="days[{{ $day }}][close_time]" value="{{ old("days.$day.close_time", '18:00') }}" 
                                                class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-800 dark:text-white">
                                    ...
                                    <input type="hidden" name="days[{{ $day }}][calendar_id]" value="{{ $day + 1 }}">
                                </div>
                            @endfor
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 pt-6">
                        ...
                        <a href="{{ route('index') }}" class="flex-1 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-800 dark:text-gray-200 font-semibold py-3 px-6 rounded-xl transition-all duration-300 text-center">
                            <svg class="w-5 h-5 mr-2 inline-block" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <script>
        ...
        const districtsByRegion = {
            ...
            "Surxandaryo": ["Termiz shahri", "Angor", "Boysun", "Denov", "Jarqo'rg'on", "Muzrabot", "Oltinsoy", "Qiziriq", "Qumqo'rg'on", "Sariosiyo", "Sherobod", "Sho'rch", "Termiz tumani", "Uzun"],
