<x-layout>
    <x-slot:title>Ustozni tahrirlash</x-slot:title>
        ...
        <div class="max-w-6xl mx-auto px-6">
            ...
            <div class="text-center mb-8">
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">
                    {{ $LearningCenter->name }}dagi ustozni tahrirlash
                </h1>
                <p class="text-gray-600 dark:text-gray-400">
                    Ustoz ma'lumotlarini yangilang
                </p>
            </div>
                                ...
                                <div class="flex flex-wrap gap-2">
                                    @foreach ($LearningCenter->connections as $connection)
                                        ...
                                        @elseif($connection->connection->name == 'Email')
                                            <a href="mailto:{{ $connection->url }}" class="p-2 bg-gray-100 dark:bg-gray-800 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors">
                    ...
                    <x-card>
                        <form action="{{ route('teacher.update', $teacher->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                            ...
                            @isset($teacher->photo)
                                <div>
                                    ...
                                    <div class="w-32 h-32 rounded-xl overflow-hidden border-2 border-gray-200 dark:border-gray-700">
                                        <img src="{{ asset('storage/' . $teacher->photo) }}" alt="Current photo" class="w-full h-full object-cover">
                                    </div>
                                </div>
                            @endisset
                            ...
                            <div>
                                ...
                                <div class="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl p-6 text-center hover:border-primary-500 transition-colors cursor-pointer" onclick="document.getElementById('photo').click()">
                                    <input type="file" name="photo" id="photo" class="hidden" accept="image/*" onchange="document.getElementById('photo-preview').src = window.URL.createObjectURL(this.files[0])">
                                    <div id="photo-preview" class="w-20 h-20 mx-auto mb-4 bg-gray-100 dark:bg-gray-800 rounded-full flex items-center justify-center">
                                        @isset($teacher->photo)
                                            <img src="{{ asset('storage/' . $teacher->photo) }}" alt="Current photo" class="w-full h-full object-cover rounded-full">
                                        @else
                            ...
                            <div>
                                ...
                                <textarea name="about" id="message" rows="4" 
                                    ...
                                    placeholder="Ustoz haqida qisqacha ma'lumot...">{{ $teacher->about }}</textarea>
                                @error('about')
                                    <p class="mt-2 text-sm text-danger-600 dark:text-danger-400">{{ $message }}</p>
                                @enderror
                            </div>
